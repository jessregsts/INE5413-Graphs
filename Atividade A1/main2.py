class Grafo:
    ...
    def add_matriz(self, v1, v2, dis, matriz):
        matriz[v2][v1] = dis
        matriz[v1][v2] = dis
                ...
                if palavra[0] == "*edges":
                    ...
                    while linha:
                        if linha != "\n":
                            ...
                            self.add_matriz((int(npalavra[0])-1), (int(npalavra[1])-1), float(npalavra[2]), self.matriz)
                            self.arestas.append(((int(npalavra[0])), (int(npalavra[1]))))
                            self.arestas.append(((int(npalavra[1])), (int(npalavra[0]))))
                        linha = grafos.readline()
                        if not linha:
                            break
                if (palavra[0] != "*vertices") and (palavra[0] != "*edges"):
                    nome = " ".join(palavra)
                    self.vertices[(int(palavra[0])-1)] = f"{nome}"
                linha = grafos.readline()
